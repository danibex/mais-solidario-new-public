...
export default function CardNoticiaMenor(props: cardNoticiaMenor) {
  return (
    <Link href={props.link || ""} className='w-[392px] h-[547px] flex flex-col justify-start items-start text-left rounded-lg'>
        {/**src alt tagNome tagCor Título Autor Data Descrição */}
        <Image className='mb-3 rounded-lg' src={props.src} width={392} height={288} alt={props.alt}/>
        <p className={`
        flex justify-center items-center text-center
        mb-3 px-4 py-1 ${props.tagCor} rounded-2xl gap-2.5
        ${props.corTexto} text-base font-bold font-['Ubuntu']
        `}>{props.tagNome}</p>
        <h3 className={`hover:underline mb-3 ${fonts.poppis} text-neutral-900 text-2xl font-bold leading-[33.60px]`}>{props.titulo}</h3>
